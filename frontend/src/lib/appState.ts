import type { DragEvent, RefObject, UIEvent } from "react";
...
export function useFieldnotesApp(composerRef: RefObject<HTMLTextAreaElement | null>) {
  ...
  const visibleRoutes = routes.filter((item) => developerMode || item !== "developer");
  ...
  async function copySourceRef() {
    if (!sourceView.response) {
      return;
    }
    await copyText(`${sourceView.response.file_path} -> ${sourceView.response.label} -> ${sourceView.locator ?? ""}`);
  ...
  }, [composerRef, streamingAnswerId, chatMessages]);
